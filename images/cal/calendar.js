...
this.wrt("<table width='250px' bgcolor='ffffff' cellpadding='0'><tr><td><div align=right><a href='javascript:calendar.hideCalendar();' title='Close'><B> X </B></a>&nbsp;</div><table width='100%' >");
this.wrt("<tr><th><a href='javascript:calendar.makeCalendar("+(previous_month_year)+","+(previous_month)+");' title='"+this.month_names[previous_month]+" "+(previous_month_year)+"'><img src='" + vpth + "images/cal/arrow_2.gif' border='0'></a></th>");
...
for(var i=current_year-25; i<current_year+1; i++) {
this.wrt("<option value='"+i+"'")
if(i == year) this.wrt(" selected='selected'");
this.wrt(">"+i+"</option>");
}
this.wrt("</select></th>");
this.wrt("<th><a href='javascript:calendar.makeCalendar("+(next_month_year)+","+(next_month)+");' title='"+this.month_names[next_month]+" "+(next_month_year)+"'><img src='" + vpth + "images/cal/arrow_3.gif' border='0'></a></th></tr>");
this.wrt("<tr class='header'>");
for(var weekday=0; weekday<7; weekday++) this.wrt("<td><font color='#ffffff'><b>"+this.weekdays[weekday]+"</b></font></td>");
this.wrt("</tr>");
...
this.wrt("</table></td></tr></table>");
this.wrt("<center><a onclick='calendar.hideCalendar();' title='Cancel'><img src='images/cancel_cal.gif' alt='Cancel' border='0'></a><!--<input type='button' value='Cancel' class='calendar-cancel' onclick='calendar.hideCalendar();' />--></center>");
...
var ht = parseInt(this.getStyle(input,'height'));
//div.style.left=(xy[0]+width+10)+"px";
//div.style.top=xy[1]+"px";
div.style.left=xy[0]+"px";
div.style.top=(xy[1]+ht+5)+"px";
